...
| `makerprobe` | `funding.maker_probe` | 🧪 Замер исполнимости лимитников по реальной ленте. v2: критерий 3 провален (см. ниже) |
...
**Остановлены:** `liqmom`, `momentum`, `spreadarb`, `fundspread`, `spreadpaper`
(доказали неработоспособность), `publicalerts` (канал остановлен 27.07 по
решению владельца — фокус на торговле).

### Текущий результат `holdtest` (n=18 из 25, 29 июля)

Исправление бага работает: **17/18 сделок дожили до начисления** против 27%
раньше. Разложение в bps от нотионала:

```
Фандинг фактический:   +14.9 bps/сделку   ← ~10% от обещанного спредом
Комиссия тейкера:      −22.0
Basis:                  −8.5
                       ──────
Итого:                 −15.7 bps/сделку

При maker 8 bps:        −1.6   (почти безубыток)
При НУЛЕВЫХ комиссиях:  +6.3   ← весь минус создаётся комиссией
```

⚠️ **Цифры даны с n, потому что вывод уже разворачивался.** На n=12 (28 июля)
basis был −14.8 против фандинга +11.9, и в документацию попало заключение
«убыточен даже при нулевых комиссиях». На n=18 неравенство обратное. Ошибка
разобрана в [`PROJECT_JOURNEY.md`](PROJECT_JOURNEY.md), фаза 22 и урок 29.
Формальный вердикт — при n=25.

### Замер мейкера (n=443)

```
joint-fill:        82%  →  adverse на пару −2.75 bps (в нашу пользу)
непарный залив:    16%  →  разрулить: +36.23 bps
вклад непарных:  +5.97 bps против порога 4  ❌ критерий 3 провален
```

Причина структурна: цена не может пройти одновременно выше нашего ask и ниже
нашего bid, поэтому любое движение заливает одну ногу и уводит вторую. Порог
задним числом не двигаем. Для перп-перп вопрос неактуален — см. выше.
Детали в [`LOOPHOLE_SEARCH.md`](LOOPHOLE_SEARCH.md), часть 2.
