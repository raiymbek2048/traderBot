...
Исследование (май-сен 2026): существует ли устойчивый край в розничном
крипто-алготрейдинге на публичных сигналах. **Проверено 15 гипотез, все закрыты
количественно. Реальными деньгами не рискнули ни разу.** Единственная рабочая
КОНСТРУКЦИЯ (delta-neutral carry, гипотеза 14) линейна по капиталу — на $500
даёт пенни, край деградирует. Ограничение не в стратегии, а в капитале.
...
Снайп сеттлмента:      69 760 начислений, 0 выше комиссии хеджа   ❌
RISEx-дифференциал:    38 дней, +1.35%/год деградирует, +$0.19    ❌ край / ✅ механизм
Листинги Upbit:        100 событий, памп +7% съедается за 1 мин   ❌
```

**Гипотеза 13 объясняет весь фандинг-трек:** фактические ставки на хеджируемых
символах живут ниже издержек. Порог входа 0.20% был выше того, что начисляется.
А «452%/год» на NVDL — артефакт предсказания: settled медиана **0.0000%**.

**Гипотеза 14 нашла рабочую конструкцию, но не край:** delta-neutral carry
RISEx↔Bybit держал хедж 38 дней (расхождение 0.06%), впервые вышел в плюс
(+$0.19), но дифференциал +1.35%/год и деградирует. Механизм исправен, доход
линеен по капиталу — на $500 это пенни.
...
| Сервис | Модуль | Роль |
|---|---|---|
| `liqrec` | `arbitrage.liq_recorder` | Рекордер ликвидаций Bybit WS |
| `funding` | `funding.run` | Скан ~680 перпов, spot+perp delta-neutral, порог 0.20% |
| `risexpaper` | `funding.risex_paper` | Дифференциал RISEx↔Bybit + paper HYPE (гипотеза 14) |
