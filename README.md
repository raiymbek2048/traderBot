...
### Главный результат 28 июля: перп-перп минус даже при нулевых комиссиях

`holdtest` (n=12) исправил найденный баг — **11/12 сделок дожили до начисления**
против 27% раньше. Но это открыло причину глубже:

```
Фандинг фактический:   +11.9 bps/сделку   ← 10% от обещанного спредом (медиана 6%)
Комиссия тейкера:      −22.0
Basis:                 −14.8
                       ──────
Итого:                 −25.0 bps/сделку

При maker 8 bps:       −10.9    При НУЛЕВЫХ комиссиях: −2.9  ← всё равно минус
```

**Basis один (−14.8) превышает весь собранный фандинг (+11.9).** Спред фандинга
3–5%/день, который мы детектируем, испаряется до момента выплаты — реализуется
10% ожидания. Оптимизировать исполнение бессмысленно, когда отрицателен доход.
