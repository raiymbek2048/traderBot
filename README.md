...
### Результат замера мейкера (n=270)

Мейкер экономит 14 bps на цикле. Замерено, чего это стоит:

```
joint-fill (обе ноги):  79%  →  adverse на пару −2.92 bps (в нашу пользу)
непарный залив:         20%  →  разрулить: медиана +29.58, p90 +73.67 bps
E[стоимость]:                   +5.23 bps  →  чисто +8.77 bps
```

Экономика положительна (+7 bps даже при худшей реализуемой политике), но
**критерий 3 провален**: вклад непарных заливов +7.53 bps против порога 4.
Причина структурна — цена не может пройти одновременно выше нашего ask и ниже
нашего bid, поэтому любое движение заливает одну ногу и уводит вторую.
Порог задним числом не двигаем; решение оставлено владельцу. Детали в
